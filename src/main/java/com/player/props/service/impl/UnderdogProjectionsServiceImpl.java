...
import com.player.props.dao.UnderdogProjectionsNbaEntity;
...
import static com.player.props.util.ProjectionUtil.underdogToDbMapper;
...
@Slf4j
@Service
public class UnderdogProjectionsServiceImpl {
  ...
  public ProjectionsResponse getPlayerProjections() {
    ...
    List<UnderdogProjectionsNbaEntity> projectionsDb = em
        .createQuery("SELECT e FROM UnderdogProjectionsNbaEntity e", UnderdogProjectionsNbaEntity.class)
        .getResultList();

    // Key: Full Name | Value: PlayerPropsEntity
    Map<FullName, List<UnderdogProjectionsNbaEntity>> playerProjMap = projectionsDb.stream()
        .collect(Collectors.groupingBy(UnderdogProjectionsNbaEntity::getFullName));

    // List of last 10 games for each player
    List<PlayerPropsFactEntity> playersLast10Games = getLastXGamesForPlayers(playerProjMap.keySet().stream().toList());
    ...
    Map<String, List<PlayerProjectionInformation>> playerProjectionInformationMap = projectionsDb.stream()
        .filter(entity -> underdogToDbMapper.containsKey(entity.getStatType()))
        .collect(Collectors.groupingBy(UnderdogProjectionsNbaEntity::getStatType,
  ...
  private void addLineHistory(Map<String, List<PlayerProjectionInformation>> playerProjectionInformationMap,
    ...
    List<PlayerProjectionInformation> increasedMultiplier = new ArrayList<>();
    ...
    for (Map.Entry<String, List<PlayerProjectionInformation>> infoMap : playerProjectionInformationMap.entrySet()) {
      ...
      for (PlayerProjectionInformation info : infoList) {
        List<PlayerPropsFactEntity> entityList = playersLast10GamesMap.get(info.getFullName());            
        entityList = entityList == null ? new ArrayList<>() : entityList;
        List<LineHistory> lineHistories = new ArrayList<>();        
        for (int i = 0; i < entityList.size(); i++) {
          double value = 0;
          value = underdogToDbMapper.get(statType).apply(entityList.get(i));
        ...
        if (entityList.size() == 0) {
          ...
          info.setOpposingTeam(getOpposingTeam(info.getOpposingTeam(), info.teamAbbreviation));
        }        
        ...
        if (info.getPayoutMultiplier() > 1.0) {
          increasedMultiplier.add(info);
        }
    ...
    increasedMultiplier.sort((a, b) -> b.getPayoutMultiplier().compareTo(a.getPayoutMultiplier()));
    playerProjectionInformationMap.put("Hot Streaks", hotStreaks);
    playerProjectionInformationMap.put("Frequently Hit", frequentlyHit);
    playerProjectionInformationMap.put("Increased Multiplier", increasedMultiplier);
    playerProjectionInformationMap.put("All Projections", allProjections);
  }

  

  private String getOpposingTeam(String teamStr, String teamAbbreviation) {
    String[] teams = teamStr.split(" @ ");
    return Arrays.stream(teams)
                 .filter(team -> !team.equals(teamAbbreviation))
                 .toArray(String[]::new)[0];    
  }

  private PlayerProjectionInformation mapToResponse(UnderdogProjectionsNbaEntity entity) {
  ...
  public List<LineHistory> getPlayerPropsAgainstOpponent(List<PlayerStatsResponse> data, String statType,
    ...
    List<LineHistory> lineHistories = playerPropsFactEntities.stream().map(entity -> {
      Double value = underdogToDbMapper.get(statType).apply(entity);
